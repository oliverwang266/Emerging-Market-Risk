...
OUTPUT_CSV_PATH = 'datastore/derived/reports/lseg/report_data_topic_prisk.csv'
...
def main():
    ...
    # Load topic-specific bigrams and calculate IDF
    topic_files = ['health_care.txt', 'tax_policy.txt', 'economic_policy_budget.txt', 'security_defense.txt',
                   'environment.txt', 'institutional_political_process.txt', 'trade.txt', 'technology_infrastructure.txt']
    topic_bigrams_list = [load_bigrams(os.path.join('datastore/derived/reports/lseg/training_library', filename)) for filename in topic_files]
    idf = calculate_idf(topic_bigrams_list)
    
    topic_bigrams_dict = dict(zip(topic_files, topic_bigrams_list))
    ...
    for file in os.listdir(INPUT_DIR):
        if file.endswith('.zip'):
            extract_from_zipfile(file, report_data, political_bigrams, non_political_bigrams, synonyms, topic_bigrams_dict, idf)
...
# =====================================================================
def extract_from_zipfile(file, report_data, political_bigrams, non_political_bigrams, synonyms, topic_bigrams_dict, idf):
    '''
    ...
    with zipfile.ZipFile(zip_path, 'r') as z:
        for filename in z.namelist():
            if not filename.endswith('/'):  # Skip directories
                extract_from_text(z, filename, report_data, political_bigrams, non_political_bigrams, synonyms, topic_bigrams_dict, idf)

def extract_from_text(zip_file, filename, report_data, political_bigrams, non_political_bigrams, synonyms, topic_bigrams_dict, idf):
    '''
    ...
    if filename.endswith('.txt'):
        with zip_file.open(filename) as f:
            ...
            topic_risks = {topic: calculate_topic_specific_risk(text, topic_bigrams, non_political_bigrams, synonyms, idf) for topic, topic_bigrams in topic_bigrams_dict.items()}
            ...
            print(f"Processed report: {report_id}, Date: {date}, Bank: {bank}, Country: {country}, PRisk: {prisk}, NPRisk: {nprisk}, Topic Risks: {topic_risks}")
...
# =====================================================================
# Define function to calculate topic-specific risk score
# =====================================================================
def calculate_topic_specific_risk(text, topic_bigrams, non_political_bigrams, synonyms, idf):
    bigrams, words = extract_bigrams(text)
    bigram_counts = Counter(bigrams)
    ...
    for i, word in enumerate(words):
        if word in synonyms:
            context_bigrams = bigrams[max(0, i-10):min(len(bigrams), i+10)]
            for bigram in context_bigrams:
                if bigram in topic_bigrams and bigram not in non_political_bigrams:
                    weight = idf.get(bigram, 0)
                    risk_count += bigram_counts[bigram] * weight
    
    risk = risk_count / total_bigrams if total_bigrams > 0 else 0
    return risk

# =====================================================================
# Define function to calculate IDF for topic-specific bigrams
# =====================================================================
def calculate_idf(topic_bigrams_list):
    document_count = len(topic_bigrams_list)
    idf = {}
    for bigrams in topic_bigrams_list:
        for bigram in bigrams:
            if bigram not in idf:
                idf[bigram] = 0
            idf[bigram] += 1
    
    for bigram, count in idf.items():
        idf[bigram] = math.log(document_count / count)
